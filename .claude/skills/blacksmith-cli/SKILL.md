...
| `blacksmith sync` | Regenerate frontend API client from the Express OpenAPI schema (fullstack only) |
...
```json
{
  ...
  "version": "0.1.0",
  "type": "fullstack",
  "backend": { "port": 8000, "framework": "express" },
  "frontend": { "port": 5173 }
}
```
...
- **Fullstack**: Runs three concurrent processes — Express, Vite, and an OpenAPI file watcher (auto-syncs types on `.ts` changes)
- **Backend**: Runs Express only
...
1. Generates the OpenAPI schema offline using `npm run openapi` — no server needs to be running
...
blacksmith init my-app --type fullstack --backend express -b 9000 -f 3000 --ai
```
